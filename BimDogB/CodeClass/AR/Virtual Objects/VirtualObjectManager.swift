...
class VirtualObjectManager {
    ...
    func fliderWithItemType(str : String, arr : Array<VirtualObjectDefinition>) -> NSArray {
    ...
    func loadVirtualObject(_ object: VirtualObject, to position: float3, cameraTransform: matrix_float4x4) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            if let audioURL = object.audioURL  {
                let audioSource = SCNAudioSource.init(url: audioURL)!
                audioSource.loops = true
                audioSource.load()
                
                let audioPlayer = SCNAudioPlayer(source: audioSource)
                object.addAudioPlayer(audioPlayer)
            }
    ...
    private func setNewVirtualObjectPosition(_ object: VirtualObject, to pos: float3, cameraTransform: matrix_float4x4) {
        ...
        print("simdPosition \(object.simdPosition)")
