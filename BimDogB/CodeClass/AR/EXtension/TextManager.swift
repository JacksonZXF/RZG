...
extension ARCamera.TrackingState {
    var presentationString: String {
        switch self {
        case .notAvailable:
            return "可能受环境因素影响， AR检测失败"
        case .normal:
            return "平面检测正常"
        case .limited(let reason):
            switch reason {
            case .excessiveMotion:
                return "平面检测受限，请保持手机摄像足够平稳"
            case .insufficientFeatures:
                return "平面检测受限， 尝试在房间内走动走动"
            case .initializing:
                return "初始化AR会话"
            }
        }
    ...
    var recommendation: String? {
        ...
        case .limited(.excessiveMotion):
            return "试着放慢你的动作，或者重置AR扫描."
        case .limited(.insufficientFeatures):
            return "试着寻找平坦的地面，或重置AR会话。"
        default:
            return nil
        }
    }
...
class TextManager {
    ...
    private var viewController: ARScanningViewController!
	...
	init(viewController: ARScanningViewController) {
